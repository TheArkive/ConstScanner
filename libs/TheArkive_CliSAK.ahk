...
;
;   CLIobj.command
;       When using mode "p", the command passed is stored in CLIobj.command
;       Only the data is in stdout when using mode "p", and if NOT using mode "r", the prompt is included.
;
...
class cli {
    ...
    delay:=10, waitTimeout:=300, cmdTimeout:=0, showWindow:=0, codepage:="CP0", workingDir:=A_WorkingDir, shell:="windows", ready:=false, run:=false
    ID:="", mode:="w", hStdIn:=0, hStdOut:=0, hStdErr:=0, stdout:="", stdoutRaw:="", stderr:="", cmdHistory:="", conWidth:=0, conHeight:=0
    firstCmd:="", lastCmd:="", cmdCmd:="", cmdSwitches:="", cmdProg:="", useAltShell := "", reason:="", command:=""
    batchCmdLines:=0, batchProgress:=0, batchCmd:="", terminateBatch := false
    fStdErr:={AtEOF:0}, fStdOut:={AtEOF:0}
    
    __New(sCmd, options:="") {
        z := StrSplit(sCmd," "), (z.Has(1)) ? cmdExists := this.cmdExists(z[1]) : false
        
        If !z.Has(1) Or !cmdExists {
            Msgbox "Invalid command or command is blank.`r`n`r`nCommand:`r`n     " (z.Has(1) ? z[1] : "")
            return
        }
        
        this.batchCmdLines := this.shellCmdLines(sCmd,firstCmd,batchCmd) ; ByRef firstCmd / ByRef batchCmd
    ...
    cmdExists(inCmd) {
        path := Trim(EnvGet("Path"),";"), cmdExists := false
        Loop Parse path, ";"
        {
            f := A_LoopField
            If (FileExist(f "\" inCmd) Or FileExist(f "\" inCmd ".exe") Or FileExist(f "\" inCmd ".bat") Or FileExist(f "\" inCmd ".com")) And f {
                cmdExists := true
                Break
            }
        }
    ...
    runCmd() {
        p := A_PtrSize, this.run := true
        (InStr(this.mode,"p") And !InStr(this.mode,"s")) ? this.mode .= "s" : ""                ; imply mode "s" for mode "p"
    ...
    close() { ; closes handles and may/may not kill process instance
        ...
        If !InStr(this.mode,"m") And Type(this.fStdOut) = "File"
    ...
    sGet() { ; stream-Get (timer) - collects until process exits AND buffer is empty
        m := InStr(this.mode,"m") ? 1 : 0, batchCmd := Trim(this.batchCmd," `r`n`t"), prompt := "", mData := 0
        ...
        ; a := StrSplit(batchCmd,"`n","`r")
        ; debug.msg("first cmd: " this.FirstCmd "`r`n`r`n" buffer "`r`n`r`nbatch: " (a.Has(1) ? a[1] : a.Length) "`r`n`r`n" )
        
        ; If (InStr(this.mode,"x")) { ; StdErr in separate stream
            ; stdErr := this.fStdErr.read()
            ; If (stdErr) {
                ; (InStr(this.mode,"e") And IsFunc(StdErrCallback)) ? %StdErrCallback%(stdErr,ID,this) : "" ; StdErr callback
        ...
        If (buffer) { ; buffer data automatically Trim()'s CRLF for better control
            ...
            If (!m) Or (m And this.stdoutRaw != buffer) { ; normal collection - when there's a buffer
                ...
                (prompt And !InStr(this.mode,"r")) ? (this.stdout .= "`r`n" prompt "`r`n`r`n") : (this.stdout .= "`r`n") ; re-insert prompt if no mode "r" is used
                
                If (InStr(this.mode,"o") And IsFunc(StdOutCallback) And Trim(buffer," `r`n`t"))
                    %StdOutCallback%((m ? Trim(buffer," `r`n") : buffer),this.ID,this) ; trigger StdOut callback
                
                ; === new ===================================
                If prompt And InStr(this.mode,"p") {
                    this.promptEvent(prompt,buffer)
                    (!this.ready) ? (this.ready := true) : ""
                }
                
                ; === old ===================================
                ; If (prompt And InStr(mode,"p") And IsFunc(PromptCallback) And this.ready)
                    ; %PromptCallback%(prompt,ID,this)
                ...
                ; If (prompt And fullEOF And batchCmd And this.ready)            ; process should be idle when prompt appears
                    ; batchCmd ? this.write(batchCmd) : ""        ; write next command in batch, if any
                ; Else
                    ; this.ready := true, this.stdout := ""
            } 
        }
    ...
    getStdErr() {
        If (InStr(this.mode,"x")) { ; StdErr in separate stream
            StdErrCallback := this.StdErrCallback, stdErr := this.fStdErr.read()
            If (stdErr) {
                (InStr(this.mode,"e") And IsFunc(StdErrCallback)) ? %StdErrCallback%(stdErr,ID,this) : "" ; StdErr callback
                this.stderr .= stdErr "`r`n`r`n"
            }
    ...
    promptEvent(prompt,buffer) {
        PromptCallback := this.PromptCallback
        If IsFunc(PromptCallback)
            %PromptCallback%(prompt,this.ID,this)
        
        (this.batchCmd) ? this.write(this.batchCmd) : ""  ; write next command in batch, if any
    }
    write(sInput:="", isSession:=false) {
        If !this.run {
        ...
        If (sInput = "") Or this.terminateBatch {
            this.firstCmd := "", this.batchCmd := "", this.terminateBatch := false, this.batchProgress := 0, this.batchCmdLines := 0, this.command := ""
            Return
        }
        ...
        If isSession
            this.batchCmdLines := this.shellCmdLines(sCmd,firstCmd,batchCmd) ; ByRef firstCmd / ByRef batchCmd ; one or the other
        Else cmdLines := this.shellCmdLines(sInput,firstCmd,batchCmd) ; ByRef firstCmd / ByRef batchCmd
        
        this.batchCmd := batchCmd, this.firstCmd := firstCmd, this.cmdHistory .= firstCmd "`r`n"
        ...
        Else f := FileOpen(this.hStdIn, "h", this.codepage), f.Write(firstCmd "`r`n"), f.close(), f := "" ; send cmd
